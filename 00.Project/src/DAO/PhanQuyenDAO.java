...
public class PhanQuyenDAO {
    ...
    public int update(PhanQuyenDTO t) {
        ...
        try {
            Connection c = JDBCUtil.getConnection();
            String sql = "update PhanQuyen set QuyenKhachHang=?,QuyenBanHang=?,QuyenNhapHang=?,QuyenMon=?,QuyenNguyenLieu=?,QuyenLichLam=?,QuyenKhuyenMaiUuDa=?,QuyenNhaCungCap=?,QuyenNhanVien=?,QuyenThongKe=? where MaPhanQuyen=? ";
            PreparedStatement pst = c.prepareStatement(sql);
            pst.setBoolean(3, t.getQuyenKhachHang());
            pst.setBoolean(4, t.getQuyenBanHang());
            pst.setBoolean(5, t.getQuyenNhapHang());
    ...
    public ArrayList<PhanQuyenDTO> selectAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String tenQuyen = rs.getString("TenPhanQuyen");
    ...
    public String getIdByName(String name) {
        ...
        try {
            Connection c = JDBCUtil.getConnection();
            String sql = "select * from PhanQuyen where TenPhanQuyen = ? and TrangThaiPhanQuyen = 1";
